 ...
 */
public final class ClAwareResourceMap
{

    /** The logger. */
    private static final Logger LOG = LoggerFactory.getLogger( ClAwareResourceMap.class );


    /**
     * Private contstructor.
     */
    private ClAwareResourceMap()
    {
    }
     ...
     */
    public static Map<String, Boolean> getResources( File location, Pattern pattern )
    {
        HashMap<String, Boolean> retval = new HashMap<String, Boolean>();

        if ( location.exists() )
        {
            getResources( retval, location.getAbsolutePath(), pattern );
        }
        else
        {
            LOG.error( "unable to load schema from the given resource value {}", location );
        }
