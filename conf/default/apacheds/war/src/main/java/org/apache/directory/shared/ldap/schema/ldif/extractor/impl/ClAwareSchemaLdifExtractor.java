 ...
 */
public class ClAwareSchemaLdifExtractor implements SchemaLdifExtractor
{
    /** The base path. */
    ...
    /** The logger. */
    private static final Logger LOG = LoggerFactory.getLogger( ClAwareSchemaLdifExtractor.class );
    ...
    private static final String RESOURCE_NAME = "/" + SCHEMA_SUBDIR + "/ou=schema.ldif";
    ...
    public static void main( String[] args ) {
        try {
            new ClAwareSchemaLdifExtractor( new File(".") ).extractOrCopy( true );
        } catch ( Exception e ) {
            e.printStackTrace();
        }
    }
     ...
     */
    public ClAwareSchemaLdifExtractor( File outputDirectory )
    {
        ...
        extracted = new File( outputDirectory, RESOURCE_NAME ).exists();
        LOG.debug( "Schema directory '{}' does {} exist", 
            schemaDirectory, extracted ? "" : "NOT ");
    }
    ...
    {
        LOG.debug( "Try to extract schema to {}.", outputDirectory);
        if ( !outputDirectory.exists() && !outputDirectory.mkdirs() )
        {
            throw new IOException( I18n.err( I18n.ERR_08001, outputDirectory
                        .getAbsolutePath() ) );
        }
        ...
        File schemaLocation = getSchemaLocation();
        if ( ( schemaLocation == null ) && !overwrite ) {
            throw new IOException( I18n.err( I18n.ERR_08001, schemaDirectory.getAbsolutePath() ) );
        }
        LOG.info( "Extract schema from {}.", schemaLocation);

        Map<String, Boolean> list = ClAwareResourceMap.getResources( schemaLocation, EXTRACT_PATTERN );
    ...
    public File getSchemaLocation() throws IOException {
        URL schemaLocation = SchemaLdifExtractor.class.getResource( RESOURCE_NAME );
        LOG.info( "Schema found at {}.", schemaLocation);
        if ( schemaLocation == null ) {
            return null;
        }
        try {
            if ( schemaLocation.getProtocol().equals( "jar" ) ) {

                String path = schemaLocation.getFile();
                path = path.substring(0, path.indexOf('!'));

                return new File(new URL(path).toURI());

            } else if ( schemaLocation.getProtocol().equals( "file" ) ) {
                // extract dir
                String path = new File( schemaLocation.toURI() ).getAbsolutePath();
                return new File( path.substring(0, path.length() - RESOURCE_NAME.length()));

            } else {
                LOG.error("unable to load schema from the given resource value {}", schemaLocation);
                return null;
            }
    ...
    private void copyFile( File source, File destination ) throws IOException
    {
        ...
        {
            throw new IOException( I18n.err( I18n.ERR_08001, destination.getParentFile()
                        .getAbsolutePath() ) );
        }
    ...
    private void extractFromClassLoader( String resource ) throws IOException
    {
        ...
        try
        {
            ...
            if ( !destination.getParentFile().exists() && !destination.getParentFile().mkdirs() )
            {
                throw new IOException( I18n.err( I18n.ERR_08001, destination
